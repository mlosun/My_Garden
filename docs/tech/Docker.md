...
- `restart: no` 容器不会自动重启，适用于不需要自动重启的容器，例如一次性任务或调试环境。（未指定时默认）  
- `restart: always` 无论容器退出状态如何，都会自动重启，适用于需要持续运行的服务，如 Web 服务器、数据库服务等。  
- `restart: on-failure` 仅在容器以非零状态退出时自动重启，适用于那些在正常运行时会以零状态退出，但在遇到错误时会以非零状态退出的容器。可选参数：指定最大重启次数 `restart: on-failure:5`  
- `restart: unless-stopped` 容器会自动重启，除非它被手动停止或 Docker 本身被停止，适用于需要持续运行的服务，但希望在手动停止后不会自动重启。  
- 不同策略之间区别：  
